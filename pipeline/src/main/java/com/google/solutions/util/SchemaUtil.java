...
/**
 * It's a copy of the non-public {@link org.apache.beam.sdk.io.jdbc.SchemaUtil}
 *
...
public class SchemaUtil {
   ...
   * Schema.Field} corresponding to JDBC field metadata.
   */
  @FunctionalInterface
  ...
  private static BeamFieldConverter jdbcTypeToBeamFieldConverter(JDBCType jdbcType) {
    switch (jdbcType) {
      ...
      case REAL:
        return beamFieldOfType(FieldType.FLOAT);
      case SMALLINT:
        return beamFieldOfType(FieldType.INT16);
      case TIME:
        return beamFieldOfType(LogicalTypes.JDBC_TIME_TYPE);
      case TIMESTAMP:
        return beamFieldOfType(FieldType.DATETIME);
      case TIMESTAMP_WITH_TIMEZONE:
        return beamFieldOfType(LogicalTypes.JDBC_TIMESTAMP_WITH_TIMEZONE_TYPE);
      case TINYINT:
        return beamFieldOfType(FieldType.BYTE);
      case VARBINARY:
  ...
  private static BeamFieldConverter beamFieldOfType(FieldType fieldType) {
    return (index, md) -> {
  ...
  private static <InputT, BaseT> BeamFieldConverter beamLogicalField(
    ...
    return (index, md) -> {
      ...
      FieldType fieldType =
          FieldType.logicalType(constructor.apply(identifier, size));
  ...
  private static BeamFieldConverter beamArrayField() {
    return (index, md) -> {
      ...
      String label = md.getColumnLabel(index);
      FieldType elementBeamType = elementFieldConverter.create(index, md).getType();
      return Schema.Field.of(label, FieldType.array(elementBeamType))
          .withNullable(md.isNullable(index) == ResultSetMetaData.columnNullable);
    };
  }

  /** Creates a {@link ResultSetFieldExtractor} for the given type. */
  private static ResultSetFieldExtractor createFieldExtractor(FieldType fieldType) {
  ...
  /**
   * A {@link JdbcIO.RowMapper} implementation that converts JDBC
   ...
   */
  static boolean compareSchemaFieldType(FieldType a, FieldType b) {
